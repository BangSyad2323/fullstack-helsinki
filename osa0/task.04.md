...
sequenceDiagram
    ...
    activate server
    Note over server: Server processes form data, creates a new note object,<br/>and pushes it to the 'notes' array
    server-->>browser: HTTP status code 302 (URL Redirect to /exampleapp/notes)
    deactivate server

    Note right of browser: The browser reloads the page as instructed by the server's redirect response
